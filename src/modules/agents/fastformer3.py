...
class FastAttention(nn.Module):
    ...
    def forward(self, g_states, agent_qs, mask = None):
        # x = [bs * n_agents, input_shape]
        n = self.args.eps_limit
        device, h, use_rotary_emb = g_states.device, self.heads, exists(self.pos_emb)
        b = 1 if g_states.shape[0] < self.args.batch_size else self.args.batch_size
        ...
        """ q = torch.abs(self.q(g_states))
        k = self.k(agent_qs)
        v = self.v(obs) """
        q = self.q(agent_qs)
        kv = self.to_kv(g_states).chunk(2, dim = -1)        
        k = kv[0]
        v = kv[1]
