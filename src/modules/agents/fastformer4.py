...
class FastAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        device, h, use_rotary_emb = x.device, self.heads, exists(self.pos_emb)
        ...
        obs = x[0][0]
        acts = x[0][1] """
        b = 1 if x.shape[0] < self.args.batch_size else self.args.batch_size
        qkv = self.to_qkv(x).chunk(3, dim = -1)
        #print('qkv: {}'.format(qkv[0].shape))
        #q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=h), qkv)
        q = qkv[0]
        k = qkv[1]
        v = qkv[2]
        #print('q: {}, k: {}, v: {}, n_agents: {}, b: {}'.format(q.shape, k.shape, v.shape, self.args.n_agents, b))
        """ q = self.q(acts)
        k = self.k(obs)
        v = self.v(obs) """
...
class FastTransformer(nn.Module):
        ...
        *,
        ...
        for _ in range(depth):
            attn = FastAttention(dim, dim_head = dim_head, heads = heads, 
                                 ...
                                 n_agents=n_agents, args=self.args)
    ...
    def forward(
        ...
        #n, device = obs.shape[1], obs.device
        #x = self.token_emb(x)
        ...
        """ if exists(self.abs_pos_emb):            
            ...
            obs = obs + pos_emb
            acts = acts + pos_emb
        x = torch.stack((obs, acts), dim=0) """
        #print(" FF s: {}, agt_out: {}, hist: {}, b_max: {}".format(states.shape, agent_outs.shape, histories.shape, self.b_max))
        #print('----')
        x1 = x.clone().detach()
        #print("bef x: {}, x1: {}".format(x.shape, x1.shape))
        #x = torch.cat([x, x1], dim=1)
        #x1 = self.bottleneck(x1)
        #print("out x: {}, x1: {}".format(x.shape, x1.shape))
        for attn, ff in self.layers:
            #attn.b_max = self.b_max
            #x = attn(x)
            #x = attn(obs, acts)
            x = attn(x).view(-1, self.dim) + x
            x = ff(x) + x
        
            #x = ff(x).view(-1, self.dim) + x1
            #print("after ff x: {}".format(x.shape))
            #x = x + rearrange(x1, '(b n) d -> b n d', b=self.args.batch_size)
            #print(" x: {}".format(x.shape))
            #x = x.view(-1, self.dim)
            #print('in x: {}, x1: {}'.format(x.shape, x1.shape))
            x = x.view(-1, self.dim) + x1 #self.bottleneck(x1)
            #x = torch.cat([x, x1], dim=1)
        x = x.view(-1, self.dim) + x1
